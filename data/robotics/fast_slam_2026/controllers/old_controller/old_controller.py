...
def multivariate_gaussian(z, mean, covariance):
    """
    Computes the probability density function of a multivariate Gaussian distribution as per book
    """
    d = len(z)  # Dimensionality (2 for x, y)
    det_cov = np.linalg.det(covariance)  # Determinant of covariance matrix
    inv_cov = np.linalg.inv(covariance)  # Inverse of covariance matrix

    diff = z - mean
    mahalanobis_term = np.dot(diff.T, np.dot(inv_cov, diff))

    # Compute Gaussian likelihood
    denominator = np.sqrt((2 * np.pi) ** d * det_cov)
    probability = np.exp(-0.5 * mahalanobis_term) / denominator
...
def eval_sensor_model(measurements, particles):
    """
    Updating landmark estimates and weights of particles
    based on sensor measurements.
    Applying the algorithm given in the canvas page.
    """
    R_t = np.array([[1.0, 0], [0, 1.0]])  # Measurement noise covariance

    total_weight = 0  # Accumulate total weight for normalization

    for particle in particles:
        for i, landmark_id in enumerate(measurements['id']):
            z_i_t = np.array([measurements['x'][i], measurements['y'][i]])

            if not particle['landmarks'][landmark_id]['observed']:
                particle['landmarks'][landmark_id]['mu'] = z_i_t + np.array([particle['x'], particle['y']])
                particle['landmarks'][landmark_id]['sigma'] = R_t
                particle['landmarks'][landmark_id]['observed'] = True
                continue

            # Get landmark estimate
            mu = particle['landmarks'][landmark_id]['mu']
            sigma = particle['landmarks'][landmark_id]['sigma']

            # Compute expected measurement
            z_hat = mu - np.array([particle['x'], particle['y']])

            # Compute Kalman Gain
            C_t = np.eye(2)  # Identity matrix for measurement model
            Q = C_t @ sigma @ C_t.T + R_t  # Innovation covariance
            K_t = sigma @ C_t.T @ np.linalg.inv(Q)

            # Update mean and covariance of landmark
            particle['landmarks'][landmark_id]['mu'] = mu + K_t @ (z_i_t - z_hat)
            particle['landmarks'][landmark_id]['sigma'] = (np.eye(2) - K_t @ C_t) @ sigma

            # Lets compute particle weight using Gaussian likelihood
            weight = multivariate_gaussian(z_i_t, mean=z_hat, covariance=Q)
            particle['weight'] *= weight  # Update weight

        total_weight += particle['weight']  # Accumulate weight for normalization

    # Normalize weights
    for particle in particles:
        if total_weight > 0:
            particle['weight'] /= total_weight
        else: # doing this for longer run when noise becomes high
            particle['weight'] = 1.0 / len(particles)  # Avoid division by zero
...
def resample_particles(particles):
    """
    Resampling step and ensuring particle diversity based on updated weights.
    """
    weights = np.array([p['weight'] for p in particles])
    weights /= np.sum(weights)  # Normalize weights

    # Cumulative sum of weights for systematic resampling
