...
class OccupancyMap(GridMapBase):
    ...
    def inverse_sensor_model(self, val, z):
        diffRange = z - self.r
        sumRange = z + self.r
        if val < diffRange:  # If the cell is in free space
            return log_odds(self.p_free)
        elif diffRange <= val <= sumRange:  # Else if the cell is at the measured range
            return log_odds(self.p_occ)
        else:  # Otherwise there is no update for cells beyond the measured range
            return log_odds(self.p_prior)
    ...
    def update_map(self, pose, scan):
        ...
        for i, z in enumerate(scan):
            # Let's compute the ray origin and its direction
            rayOrigin = theta + i * (np.pi / len(scan)) - (np.pi / 2)
            rayDirection = [np.cos(rayOrigin), np.sin(rayOrigin)]

            # Now, get the visited cells along the ray
            visited, distances = self.ray_traversal([x, y], rayDirection)

            # Lastly lets update each visited cell
            for j, cell in enumerate(visited):
                self.grid[cell[0], cell[1]] += self.inverse_sensor_model(distances[j], z)
...
class ReflectanceMap(GridMapBase):
    ...
    def update_map(self, pose, scan):
        ...
        for i, z in enumerate(scan):
            # Let's compute the ray direction
            rayOrigin = theta + i * (np.pi / len(scan)) - (np.pi / 2)
            rayDirection = [np.cos(rayOrigin), np.sin(rayOrigin)]

            # Then finding the visited cells along the ray
            visited, distances = self.ray_traversal([x, y], rayDirection)

            # Now finally update the cells along the ray as misses
            for j, cell in enumerate(visited):
                if distances[j] < z:
                    self.miss_map[cell[0], cell[1]] += 1
                elif abs(distances[j] - z) <= self.r:
                    self.hit_map[cell[0], cell[1]] += 1
                    break
