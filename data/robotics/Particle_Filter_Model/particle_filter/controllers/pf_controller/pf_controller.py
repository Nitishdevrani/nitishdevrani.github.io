...
def distance_to_closest_border(x, y, theta, map_limits):
    ...
    beam_dir_y = np.sin(theta)
    t_values = []
    # check intersection with vertical borders
    if not math.isclose(beam_dir_x, 0.0):
        # left border
        t_left = (map_limits[0] - x) / beam_dir_x
        y_left = y + t_left * beam_dir_y
        if (t_left >= 0.0) and (y_left >= map_limits[2]) and (y_left <= map_limits[3]):
            t_values.append(t_left)
        # right border
        t_right = (map_limits[1] - x) / beam_dir_x
        y_right = y + t_right * beam_dir_y
        if (t_right >= 0.0) and (y_right >= map_limits[2]) and (y_right <= map_limits[3]):
            t_values.append(t_right)
    # check intersection with horizontal borders
    if not math.isclose(beam_dir_y, 0.0):
        # bottom border
        t_bottom = (map_limits[2] - y) / beam_dir_y
        x_bottom = x + t_bottom * beam_dir_x
        if (t_bottom >= 0.0) and (x_bottom >= map_limits[0]) and (x_bottom <= map_limits[1]):
            t_values.append(t_bottom)
        # top border
        t_top = (map_limits[3] - y) / beam_dir_y
        x_top = x + t_top * beam_dir_x
        if (t_top >= 0.0) and (x_top >= map_limits[0]) and (x_top <= map_limits[1]):
...
def resample_particles(particles, weights):
    ...
    new_weights = [weights[i] for i in indexes]
...
def add_random_particles(particles, weights, map_limits):
    ...
    # get indexes of particles with lowest weights
    indexes = np.argsort(weights)[:n_replace]
    for idx in indexes:
        particles[idx] = sample_random_particle(map_limits)
    return particles
