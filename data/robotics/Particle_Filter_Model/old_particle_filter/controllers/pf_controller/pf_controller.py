...
def sample_motion_model(odometry, particles):
    ...
    # Handle small movement cases
    if abs(delta_trans) < 1e-5:
        delta_trans = 0
    if abs(delta_rot1) < 1e-5:
        delta_rot1 = 0
    if abs(delta_rot2) < 1e-5:
        delta_rot2 = 0

    # standard deviations of motion noise
    sigma_delta_rot1 = max(0, noise[0] * abs(delta_rot1) + noise[1] * delta_trans)
    sigma_delta_trans = max(0, noise[2] * delta_trans + noise[3] * (abs(delta_rot1) + abs(delta_rot2)))
    sigma_delta_rot2 = max(0, noise[0] * abs(delta_rot2) + noise[1] * delta_trans)

    # Debugging prints
    print("delta_rot1:", delta_rot1)
    print("delta_rot2:", delta_rot2)
    print("delta_trans:", delta_trans)
    print("sigma_delta_rot1:", sigma_delta_rot1)
    print("sigma_delta_trans:", sigma_delta_trans)
    print("sigma_delta_rot2:", sigma_delta_rot2)
...
def main():
    ...
    last_left_enc = leftSensor.getValue()
    last_right_enc = rightSensor.getValue()
    ...
    while robot.step(timestep) != -1:
        ...
        # compute odometry from wheel encoder readings
        odometry, last_left_enc, last_right_enc = get_odometry_from_encoders(
            leftSensor, rightSensor, last_left_enc, last_right_enc
        )

        # get current lidar measurements
        scan = lidar.getRangeImage()
        print("Lidar scan result:", scan)
        scan.reverse()

        # insert random particles to maintain diversity
        particles = add_random_particles(particles, weights, map_limits)

        # predict particles based on the odometry and motion model
        particles = sample_motion_model(odometry, particles)

        # calculate importance weights according to the sensor model
        weights = eval_sensor_model(scan, particles, obstacles, map_limits)

        # resample particle set according to weights
